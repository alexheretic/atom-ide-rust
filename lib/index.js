...
/**
 * @param {string} projectPath
 * @return {string?} input project path if valid
 */
async function validRootProject(projectPath) {
  return new Promise(resolve => {
    fs.access(path.join(projectPath, "Cargo.toml"), fs.constants.F_OK, err => {
      resolve(!err && projectPath)
    })
  })
}

async function singleChildDirWithCargoToml(projectPath) {
  ...
  let rootPromises = children.map(dir => validRootProject(path.join(projectPath, dir)))
  let roots = (await Promise.all(rootPromises)).filter(dir => dir)
...
class RustLanguageClient extends AutoLanguageClient {
  constructor() {
    ...
    this.projects = {}
    /** (projectPath -> child dir) mappings */
    this.childProjectPaths = {}
  ...
  getInitializeParams(projectPath, childProcess) {
    let actualProjectPath = this.childProjectPaths[projectPath] || projectPath
    const params = super.getInitializeParams(actualProjectPath, childProcess) || {}
  ...
  async startServerProcess(projectPath) {
    ...
    // If no Cargo.toml is present try to find a single child directory that has one
    let actualProjectPath = await validRootProject(projectPath)
      || await singleChildDirWithCargoToml(projectPath)
      || projectPath

    if (shouldIgnoreProjectPath(actualProjectPath)) {
      console.warn("ide-rust disabled on", actualProjectPath)
      return
    }

    if (projectPath !== actualProjectPath) {
      // Using a child dir with a Cargo.toml as project path
      this.childProjectPaths[projectPath] = actualProjectPath
    }
